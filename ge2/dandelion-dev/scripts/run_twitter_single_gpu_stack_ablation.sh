...
CONFIG_SRC="${CONFIG_SRC:-$REPO_ROOT/gege/configs/single_gpu/twitter_16p.yaml}"
DATASET_DIR="${DATASET_DIR:-}"
LOG_DIR="${LOG_DIR:-$REPO_ROOT/experiment_logs}"
OUTPUT_ROOT="${OUTPUT_ROOT:-/dev/shm/smansou2_ge2/twitter_single_gpu_stack_ablation}"
...
stack_cases=(
  stack_00_control
  stack_01_bitmap_unique
  stack_02_plus_emulate_dot_single_relation
  stack_03_plus_mem_partition_buffer_pinned_host
  stack_04_plus_fast_map_tensors
  stack_05_plus_partition_buffer_lp_fast_path
  stack_06_plus_single_gpu_gpu_aware_custom
  stack_07_plus_keep_storage_hot_between_epochs
  stack_08_plus_gpu_active_edge_shuffle
  stack_09_plus_deg_chunk_exclusion
)
...
flags_string_for_case() {
  case "$1" in
    stack_00_control)
      ...
      ;;
    stack_01_bitmap_unique)
      printf '%s' 'GEGE_UNIQUE_BACKEND=bitmap, GEGE_UNIQUE_BITMAP_NUM_NODES=41652230'
      ;;
    stack_02_plus_emulate_dot_single_relation)
      printf '%s' 'previous_stack + GEGE_EMULATE_DOT_SINGLE_RELATION=1'
      ;;
    stack_03_plus_mem_partition_buffer_pinned_host)
...
s|^[[:space:]]*save_state: .*|    save_state: false|m;" \
  ...
  python3 "$UPDATE_SCRIPT" \
    ...
    --row "$case_name" \
    --branch main \
    --config "gege/configs/single_gpu/twitter_16p.yaml" \
    --flags "$(flags_string_for_case "$case_name")" \
    --epochs "$EPOCHS" \
    --train-log "experiment_logs/${run_name}_train.log" \
    --eval-log "n/a" \
    --eval-notes "train only; eval skipped" \
    --notes "${EPOCHS}-epoch Twitter single-GPU train-only stack run via run_twitter_single_gpu_stack_ablation.sh" \
    --summary "$summary_log"
done
