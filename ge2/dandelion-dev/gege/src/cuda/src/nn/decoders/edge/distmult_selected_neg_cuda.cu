...
__global__ void distmult_selected_neg_scores_kernel(const scalar_t* __restrict__ chunked_adjusted_embeddings,
...
template <typename scalar_t>
__global__ void distmult_selected_neg_grad_chunked_kernel(const scalar_t* __restrict__ grad_out,
                                                          const scalar_t* __restrict__ negative_embeddings,
                                                          const int64_t* __restrict__ selected_neg_indices,
...
template <typename scalar_t>
__global__ void distmult_selected_neg_grad_negative_kernel(const scalar_t* __restrict__ grad_out,
...
torch::Tensor distmult_selected_neg_scores_cuda_forward(torch::Tensor chunked_adjusted_embeddings,
    ...
    AT_DISPATCH_FLOATING_TYPES_AND_HALF(chunked.scalar_type(), "distmult_selected_neg_scores_cuda_forward", [&] {
        distmult_selected_neg_scores_kernel<scalar_t><<<blocks, threads, 0, stream>>>(
...
std::tuple<torch::Tensor, torch::Tensor> distmult_selected_neg_scores_cuda_backward(torch::Tensor grad_out,
    ...
    AT_DISPATCH_FLOATING_TYPES_AND_HALF(chunked.scalar_type(), "distmult_selected_neg_scores_cuda_backward", [&] {
        distmult_selected_neg_grad_chunked_kernel<scalar_t><<<grad_chunked_blocks, threads, 0, stream>>>(
        ...
        distmult_selected_neg_grad_negative_kernel<scalar_t><<<grad_negative_blocks, threads, 0, stream>>>(
            grad.data_ptr<scalar_t>(),
            chunked.data_ptr<scalar_t>(),
            selected.data_ptr<int64_t>(),
