...
OUTPUT_ROOT="${OUTPUT_ROOT:-/dev/shm/smansou2_ge2/lj_single_gpu_stack_ablation}"
...
reset_env() {
  ...
  # Fixed-off block for every LJ stack run.
...
flags_string_for_case() {
  case "$1" in
    stack_00_control)
      ...
      ;;
    stack_01_bitmap_unique)
      printf '%s' 'GEGE_UNIQUE_BACKEND=bitmap, GEGE_UNIQUE_BITMAP_NUM_NODES=4847571'
      ;;
    stack_02_plus_emulate_dot_single_relation)
...
s|^[[:space:]]*save_state: .*|    save_state: false|m;" \
  ...
  python3 "$UPDATE_SCRIPT" \
    ...
    --row "$case_name" \
    --branch main \
    --config "gege/configs/single_gpu/livejournal_16p.yaml" \
    --flags "$(flags_string_for_case "$case_name")" \
    --epochs "$EPOCHS" \
    --train-log "experiment_logs/${run_name}_train.log" \
    --eval-log "n/a" \
    --eval-notes "train only; eval skipped" \
    --notes "5-epoch LJ single-GPU train-only stack run via run_lj_single_gpu_stack_ablation.sh" \
    --summary "$summary_log"
done
