...
BUILD_DIR="${BUILD_DIR:-$REPO_ROOT/build_4gpu_runs}"
CONFIG_SRC="${CONFIG_SRC:-$REPO_ROOT/gege/configs/4gpu/twitter_16p.yaml}"
LOG_DIR="${LOG_DIR:-$REPO_ROOT/experiment_logs}"
OUTPUT_ROOT="${OUTPUT_ROOT:-/dev/shm/smansou2_ge2/twitter_4gpu_stack_ablation}"
RESULTS_MD="${RESULTS_MD:-$REPO_ROOT/4gpu_ablation_results_template.md}"
...
CLEANUP_RUN_OUTPUTS="${CLEANUP_RUN_OUTPUTS:-1}"
...
s|^[[:space:]]*save_state: .*|    save_state: false|m;" \
  ...
  python3 "$UPDATE_SCRIPT" \
    ...
    --row "$case_name" \
    --branch main \
    --config "gege/configs/4gpu/twitter_16p.yaml" \
    --flags "$(flags_string_for_case "$case_name")" \
    --epochs "$EPOCHS" \
    --train-log "experiment_logs/${run_name}_train.log" \
    --eval-log "n/a" \
    --eval-notes "train only; eval skipped" \
    --notes "Twitter 4-GPU importance stack run via run_twitter_4gpu_stack_ablation.sh" \
    --summary "$summary_log"

  cleanup_run_artifacts "$run_root" "$config_tmp"
done
