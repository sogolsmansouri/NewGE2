...
CONFIG_SRC="${CONFIG_SRC:-$REPO_ROOT/gege/configs/4gpu/livejournal_16p.yaml}"
LOG_DIR="${LOG_DIR:-$REPO_ROOT/experiment_logs}"
OUTPUT_ROOT="${OUTPUT_ROOT:-/dev/shm/smansou2_ge2/lj_4gpu_stack_ablation}"
...
s|^[[:space:]]*save_state: .*|    save_state: false|m;" \
  ...
  python3 "$UPDATE_SCRIPT" \
    ...
    --row "$case_name" \
    --branch main \
    --config "gege/configs/4gpu/livejournal_16p.yaml" \
    --flags "$(flags_string_for_case "$case_name")" \
    --epochs "$EPOCHS" \
    --train-log "experiment_logs/${run_name}_train.log" \
    --eval-log "n/a" \
    --eval-notes "train only; eval skipped" \
    --notes "LJ 4-GPU importance stack run via run_lj_4gpu_stack_ablation.sh with GEGE_EMULATE_DOT_SINGLE_RELATION=1 fixed across runs" \
    --summary "$summary_log"
